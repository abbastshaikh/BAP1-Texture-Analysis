...
def preprocess (image, mask, 
    ...
    if discretize:
        ...
        if discretize == "fixedwidth":
            modifiedImage = radiomics.imageoperations.binImage(parameterMatrix,
        ...
        modifiedImage = sitk.GetImageFromArray(modifiedImage)
        modifiedImage.SetDirection(image.GetDirection())
        modifiedImage.SetSpacing(image.GetSpacing())
        modifiedImage.SetOrigin(image.GetOrigin())
    ...
    if preprocessing_filter == "LoG":
        modifiedImage = sitk.LaplacianRecursiveGaussian(modifiedImage, sigma = LoG_sigma)
        
    # Apply morphological operations to mask
    if mask_opening:
        mask = sitk.BinaryMorphologicalOpening(mask)
        
    return image, modifiedImage, mask
