...
import numpy as np
from FeatureExtractionUtil import extractFeatures, str2bool, preprocess
...
parser.add_argument('--average_across_slices', type=str2bool, default = True,
...
parser.add_argument('--radiomic_features', type=str2bool, default = True,
                    help = 'Extract Radiomics features - (y/n)')
parser.add_argument('--radiomic_feature_classes', type=str, nargs = "+", default = ['firstorder', 'glcm', 'gldm', 'glrlm', 'glszm', 'ngtdm'],
                    help = 'Texture features to extract via radiomics features extractor')
...
for feature_class in args.radiomic_feature_classes:
...
for caseIdx in range(len(cases)):
    ...
    if not args.average_across_slices:
